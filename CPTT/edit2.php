<?php
/************************************************************
 * CPTT/edit2.php — Full editor w/ Status dropdown + Audit log
 * - Auth (auth/ is sibling of CTT/)
 * - Single, mobile-friendly form
 * - Yes/No checkboxes for access flags
 * - Parameterized updates + upserts for child tables
 * - Writes Audit rows per changed field (UpdatedBy if column exists)
...
/* Treat these as Yes/No checkboxes */
$BOOLEAN_FIELDS = [
...
/* Load current record and keep a deep copy for auditing */
...
function yesNoChecked($current){
  ...
  return in_array($v, ['1','Y','YES','TRUE','ON','YES'], true);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  foreach ($fieldToTable as $col => $table) {
    ...
    if (array_key_exists($name, $_POST)) {
      ...
      $rec[$col] = $val; // also update in-memory copy for auditing comparison
    } else {
  ...
  /* Begin transaction and write changes */
  if (!sqlsrv_begin_transaction($conn)) {
    $err = 'Could not start DB transaction.';
  } else {

    // 1) Update dbo.PersonnelInfo (Status is dropdown)
...
/* ===== UI helpers ===== */
function renderText($table,$col,$rec){ echo '<input type="text" name="'.h($table.'__'.$col).'" value="'.h($rec[$col] ?? '').'" class="in in-text">'; }
function renderDate($table,$col,$rec){ echo '<input type="date" name="'.h($table.'__'.$col).'" value="'.h($rec[$col] ?? '').'" class="in in-date">'; }
function renderYesNo($table,$col,$rec){
  $name = $table.'__'.$col;
  $checked = yesNoChecked($rec[$col] ?? '') ? ' checked' : '';
...
function renderStatus($current, $name, $options){
  echo '<select name="'.h($name).'" class="in in-select">';
...
<head>
  ...
  <!-- (Optional) Bootstrap CSS for modal styling; remove if you already include it globally -->
  ...
  <style>
    ...
    .in-select { max-width: 320px; }
    .chk { display:inline-flex; gap:8px; align-items:center; }
    .actions { margin-top:14px; display:flex; gap:var(--gap); flex-wrap:wrap; justify-content:space-between; }
    .btnp { padding:10px 16px; background:#1565c0; color:#fff; border:0; border-radius:6px; cursor:pointer; text-decoration:none; }
    .btnp.secondary { background:#666; }
  </style>
</head>
<body>
<div class="wrap">
  <div class="card">
    ...
    <div class="actions">
      <a class="btnp secondary" href="dashboard.php">Back to Dashboard</a>
      <button type="button" class="btnp" data-toggle="modal" data-target="#AuditTable">Audit Table</button>
    </div>
    ...
    <form method="post" action="?Tracking_Num=<?php echo urlencode($Tracking_Num); ?>">
      ...
      <table class="kv">
        ...
        <tr><th>Email</th><td><?php renderText('dbo.PersonnelInfo','Email',$rec); ?></td></tr>
        <tr><th>Business Need</th><td><?php renderText('dbo.PersonnelInfo','Business_Need',$rec); ?></td></tr>
        <tr><th>SSN Validation Date</th><td><?php renderDate('dbo.PersonnelInfo','SSN_Validation_Date',$rec); ?></td></tr>
        <tr><th>7-Year Criminal History Check</th><td><?php renderDate('dbo.PersonnelInfo','Criminal_Background_Date',$rec); ?></td></tr>
        <tr><th>Current Training Date</th><td><?php renderDate('dbo.PersonnelInfo','CurrentTrainingDate',$rec); ?></td></tr>
        <tr><th>Paperwork Approved On</th><td><?php renderDate('dbo.PersonnelInfo','DatePaperWorkSign',$rec); ?></td></tr>
      ...
      <div class="grid">
        <table class="kv">
          ...
          <tr><th>Energy Control Center</th><td><?php renderYesNo('dbo.PhysicalAccess','ECC',$rec); ?></td></tr>
          <tr><th>ECDA Offices</th><td><?php renderYesNo('dbo.PhysicalAccess','ECDA_Offices',$rec); ?></td></tr>
          <tr><th>ECMS Offices</th><td><?php renderYesNo('dbo.PhysicalAccess','ECMS_Offices',$rec); ?></td></tr>
          <tr><th>Operations Data Center</th><td><?php renderYesNo('dbo.PhysicalAccess','Operations_Data_Center',$rec); ?></td></tr>
          <tr><th>Server Lobby / Basement</th><td><?php renderYesNo('dbo.PhysicalAccess','Server_Lobby',$rec); ?></td></tr>
          <tr><th>SNOC</th><td><?php renderYesNo('dbo.PhysicalAccess','SNOC',$rec); ?></td></tr>
        </table>
        <table class="kv">
          <tr><th>Jackson Gate</th><td><?php renderYesNo('dbo.PhysicalAccess','JacksonGate',$rec); ?></td></tr>
          <tr><th>Restricted Key</th><td><?php renderYesNo('dbo.PhysicalAccess','Restricted_Key',$rec); ?></td></tr>
          <tr><th>LAW Perimeter</th><td><?php renderYesNo('dbo.PhysicalAccess','LAW_Perimeter',$rec); ?></td></tr>
          <tr><th>LAW Data Center</th><td><?php renderYesNo('dbo.PhysicalAccess','LAW_Data_Center',$rec); ?></td></tr>
          <tr><th>LAW SNOC</th><td><?php renderYesNo('dbo.PhysicalAccess','LAW_SNOC',$rec); ?></td></tr>
          <tr><th>LAW Generation</th><td><?php renderYesNo('dbo.PhysicalAccess','LAW_Generation',$rec); ?></td></tr>
          <tr><th>LAW Transmission</th><td><?php renderYesNo('dbo.PhysicalAccess','LAW_Transmission',$rec); ?></td></tr>
          <tr><th>LAW Maintenance &amp; Electric</th><td><?php renderYesNo('dbo.PhysicalAccess','LAW_Maintenance_Electric',$rec); ?></td></tr>
          <tr><th>LAW Operations Storage</th><td><?php renderYesNo('dbo.PhysicalAccess','LAW_Operations_Storage',$rec); ?></td></tr>
          <tr><th>LAW Network Room 104</th><td><?php renderYesNo('dbo.PhysicalAccess','LAW_Network_Room_104',$rec); ?></td></tr>
        </table>
      </div>
      ...
      <div class="grid">
        <table class="kv">
          ...
          <tr><th>VPN Tunnel Access (GE)</th><td><?php renderYesNo('dbo.XA21_ECS','VPN_Tunnel_Access',$rec); ?></td></tr>
          <tr><th>Active Directory (gsoc_prod)</th><td><?php renderYesNo('dbo.XA21_ECS','AD_prod',$rec); ?></td></tr>
          <tr><th>Active Directory (gsoc_support)</th><td><?php renderYesNo('dbo.XA21_ECS','AD_supp',$rec); ?></td></tr>
          <tr><th>UNIX Access</th><td><?php renderYesNo('dbo.XA21_ECS','UNIX_Access',$rec); ?></td></tr>
          <tr><th>Internal EnterNet</th><td><?php renderYesNo('dbo.XA21_ECS','Internal_EnterNet',$rec); ?></td></tr>
          <tr><th>External EnterNet (Non-CIP)</th><td><?php renderYesNo('dbo.XA21_ECS','External_EnterNet',$rec); ?></td></tr>
          <tr><th>Database User</th><td><?php renderYesNo('dbo.XA21_ECS','Database_User',$rec); ?></td></tr>
          <tr><th>AutoCAD User</th><td><?php renderYesNo('dbo.XA21_ECS','AutoCAD_User',$rec); ?></td></tr>
          <tr><th>Sudo (root)</th><td><?php renderYesNo('dbo.XA21_ECS','Sudo_root',$rec); ?></td></tr>
          <tr><th>Sudo (xa21)</th><td><?php renderYesNo('dbo.XA21_ECS','Sudo_XA21',$rec); ?></td></tr>
          <tr><th>Sudo (xacm)</th><td><?php renderYesNo('dbo.XA21_ECS','Sudo_xacm',$rec); ?></td></tr>
          <tr><th>Sudo (oracle)</th><td><?php renderYesNo('dbo.XA21_ECS','Sudo_oracle',$rec); ?></td></tr>
          <tr><th>Sudo (ccadmin)</th><td><?php renderYesNo('dbo.XA21_ECS','Sudo_ccadmin',$rec); ?></td></tr>
          <tr><th>Admin/Shared/Generic (iccpadmin)</th><td><?php renderYesNo('dbo.XA21_ECS','AdminSharedGeneric_iccpadmin',$rec); ?></td></tr>
          <tr><th>Domain Admin</th><td><?php renderYesNo('dbo.XA21_ECS','Domain_Admin',$rec); ?></td></tr>
          <tr><th>Shared (emrg) Account</th><td><?php renderYesNo('dbo.XA21_ECS','emrg',$rec); ?></td></tr>
        </table>

        <table class="kv">
          <tr><th>TE Engineering OM Group</th><td><?php renderYesNo('dbo.NetworkDevices','TE_Engineering_OM_Group',$rec); ?></td></tr>
          <tr><th>Telecom Shared Account</th><td><?php renderYesNo('dbo.NetworkDevices','TelecomSharedAccount',$rec); ?></td></tr>
          <tr><th>ACS Local Admin</th><td><?php renderYesNo('dbo.NetworkDevices','ACS_LocalAdmin',$rec); ?></td></tr>
          <tr><th>RSA Local Admin</th><td><?php renderYesNo('dbo.NetworkDevices','RSA_LocalAdmin',$rec); ?></td></tr>

          <tr><th>ID ASA</th><td><?php renderYesNo('dbo.IndustrialDefender','IDAppAdmin',$rec); ?></td></tr>
          <tr><th>ID ASM</th><td><?php renderYesNo('dbo.IndustrialDefender','IDSysAdmin',$rec); ?></td></tr>
          <tr><th>ID NIDS</th><td><?php renderYesNo('dbo.IndustrialDefender','IDUser',$rec); ?></td></tr>
          <tr><th>ID (root) Shared</th><td><?php renderYesNo('dbo.IndustrialDefender','IDroot',$rec); ?></td></tr>
          ...
          <tr><th>CCTV Video App Admin</th><td><?php renderYesNo('dbo.PSS','CCTV_Video_Application_Administrator',$rec); ?></td></tr>
          <tr><th>CCTV Video User</th><td><?php renderYesNo('dbo.PSS','CCTV_Video_User',$rec); ?></td></tr>
          <tr><th>PSS WinAdmin</th><td><?php renderYesNo('dbo.PSS','PSS_WinAdmin',$rec); ?></td></tr>

          <tr><th>Nessus App Admin</th><td><?php renderYesNo('dbo.Nessus','NessusAppAdmin',$rec); ?></td></tr>
          <tr><th>Nessus Sys Admin</th><td><?php renderYesNo('dbo.Nessus','NessusSysAdmin',$rec); ?></td></tr>

          <tr><th>OCRS ECMS Admin</th><td><?php renderYesNo('dbo.OCRS','OCRS_ECMSAdmin',$rec); ?></td></tr>
          <tr><th>OCRS SSIT Admin</th><td><?php renderYesNo('dbo.OCRS','OCRS_SSITAdmin',$rec); ?></td></tr>
          <tr><th>OCRS User</th><td><?php renderYesNo('dbo.OCRS','OCRS_User',$rec); ?></td></tr>
          <tr><th>CIP-Protected Info</th><td><?php renderYesNo('dbo.OCRS','CIP_ProtectedInfo',$rec); ?></td></tr>
          <tr><th>Stratus</th><td><?php renderYesNo('dbo.OCRS','Stratus',$rec); ?></td></tr>
          <tr><th>Catalogic</th><td><?php renderYesNo('dbo.OCRS','Catalogic',$rec); ?></td></tr>
          <tr><th>SolarWinds</th><td><?php renderYesNo('dbo.OCRS','SolarWinds',$rec); ?></td></tr>
          <tr><th>ServiceDesk Plus</th><td><?php renderYesNo('dbo.OCRS','ServiceDeskPlus',$rec); ?></td></tr>
        </table>
      </div>

      <div class="actions">
        <button class="btnp" type="submit" name="submit" value="1">Save</button>
        <a class="btnp secondary" href="dashboard.php">Cancel</a>
      </div>
    </form>
  </div>
</div>
...
<!-- Audit Modal -->
<div class="modal fade" id="AuditTable" role="dialog" aria-hidden="true">
...
<!-- (Optional) Bootstrap JS; remove if already included globally -->
